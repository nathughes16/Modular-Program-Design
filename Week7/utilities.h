...
int read_file(FILE *inputFile, reading *dataArray)
{
    ...
    reading R;
    while(fgets(lineBuffer, bufferSize, inputFile) != NULL){
        char date;
        float bloodIron
        tokeniseRecord(lineBuffer, ",", date, bloodIron);
        strcpy(R.date, date);
        R.bloodIron = bloodIron;
        dataArray[n] = R;
        n++;
    }
    return n;
...
int data_checker(reading *dataArray, int numReadings)
{
    for(int i = 0; i < numReadings; i++){
        if(dataArray[i].date == NULL || dataArray[i].bloodIron == NULL){
            return 1;
        }
    }
    return 0;
...
{
    float max = dataArray[0].bloodIron;
...
void monthly_iron(reading* dataArray, int numReadings)
{
    ...
    char* token;
    for(int i = 0; i < numReadings; i++){
        token = strtok(numReadings[i].date, "-");
        if(strcmp(token[1], month) == 0){
            printf("%f\n", numReadings[i].bloodIron);
        }
    }
}